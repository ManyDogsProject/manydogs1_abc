...
# Needed for Arial font in Linux; comment out if not needed
extrafont::loadfonts()
...
```{r bootstrap-cis-pilot, eval = TRUE}
...
```{r bayes-factors-pilot, eval = TRUE}
...
```{r check-assumptions-pilot, eval = TRUE}
...
```{r model-building-bias, eval = TRUE}
...
```{r random-slope-breed, eval = TRUE, include = FALSE}
...
```{r run-model-breed, eval = TRUE, include = FALSE}
...
```{r bootstrap-cis-breed, eval = TRUE}
...
```{r bayes-factors-breed, eval = TRUE}
# Run the full Bayesian models with and without correlations for pilot data
# Output is saved locally to save time, so this code chunk is not evaluated during knitting
...
```{r prepare-heritability, eval = TRUE}
...
```{r run-heritability-model, eval = TRUE}
